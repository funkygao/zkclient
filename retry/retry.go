...
func RetryWithBackoff(opts RetryOptions, fn func() (RetryStatus, error)) error {
	...
	for count := 1; true; count++ {
		...
		if err != nil && (!opts.UseV1Info) {
			log.Info("%s failed an iteration: %s %s", opts.Tag, err, debug.CallerFuncName(4))
		}
		...
		if status == RetryReset {
			...
			count = 0
			if !opts.UseV1Info {
				log.Info("%s failed; retrying immediately", opts.Tag)
			}
		} else {
			...
			if !opts.UseV1Info {
				log.Info("%s failed; retrying in %s", opts.Tag, backoff)
			}
